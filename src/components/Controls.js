...
const Controls = (props) => {
  ...
  const handleClick = (value) => {
    switch (value) {
      ...
      case '%':
        var answer 
        if (isSymbolClick) {
          const newInput = input.slice(0, input.length );
  ...
  // This is where the return statement should be
  return (
    <div className="controlContainer">
      {operators.map((value, i) => <ControlButton key={i} index={i} value={value} handleClick={handleClick} />)}
    </div>
  );
}
